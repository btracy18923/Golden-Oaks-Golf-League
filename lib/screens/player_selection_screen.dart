...
class PlayerSelectionScreen extends StatefulWidget {
  ...
  const PlayerSelectionScreen({super.key, this.currentLeague});
...
class _PlayerSelectionScreenState extends State<PlayerSelectionScreen> {
  ...
  bool isLoading = true;
  late Color _leagueColor;
  
  @override
  void initState() {
    super.initState();
    // Cache league color once
    _leagueColor = widget.currentLeague == League.monday 
        ? const Color.fromRGBO(179, 255, 179, 1) // Light green
        : const Color.fromRGBO(255, 214, 0, 1); // Gold
    loadPlayers();
  }
  ...
  Future<void> loadPlayers() async {
    try {
      ...
      if (widget.currentLeague != null) {
        loadedPlayers = await _dbHelper.getPlayersByLeague(widget.currentLeague!);
      } else {
        loadedPlayers = await _dbHelper.getAllPlayers();
      }
  ...
  String get leagueText {
    if (widget.currentLeague == League.monday) {
      return "Select Players for Monday's Match";
    } else if (widget.currentLeague == League.wednesday) {
      return "Select Players for Wednesday's Match";
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(leagueText),
        backgroundColor: widget.currentLeague == League.monday 
            ? Colors.green[700] 
            : Colors.orange[700],
  ...
  void _navigateToEnterScores() {
    try {
      ...
      // Convert League enum to string
      String leagueType = widget.currentLeague == League.monday ? 'monday' : 'wednesday';
      
      // Navigate to Enter Scores Screen
      Navigator.push(
        context,
        MaterialPageRoute(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error navigating to scores: $e'),
          backgroundColor: Colors.red,
        ),
      );
    }
