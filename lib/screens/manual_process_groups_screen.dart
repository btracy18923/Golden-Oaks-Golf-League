...
class ManualProcessGroupsScreen extends StatefulWidget {
  ...
  const ManualProcessGroupsScreen({
    Key? key,
    this.initialPlayers,
    this.initialGroups,
    this.initialLeague,
  ...
  @override
  _ManualProcessGroupsScreenState createState() => _ManualProcessGroupsScreenState();
}
...
class _ManualProcessGroupsScreenState extends State<ManualProcessGroupsScreen> {
  ...
  Future<void> _assignGroupPlacesAndPrizes(List<Map<String, dynamic>> groupRankings) async {
    ...
    try {
      ...
      print('Error loading Group_Payouts.csv or calculating prizes: $e');
  ...
  Widget _buildTitleContainer() {
    return Container(
      ...
      child: Row(
        children: [
          Text(
            groupsProcessed ? "Manual Process:" : "Manual Process:",
            style: TextStyle(
              fontSize: 20,
      ...
      child: Center(
        child: Row(
          ...
          children: [
            ...
            _buildFooterButton("Process Individuals", Colors.grey[300]!, _processIndividuals),
  ...
  Future<PayoutValidationResult?> validateManualGroupProcessingResults() async {
    try {
      // Get total number of selected players (excluding wild cards)
      int totalSelectedPlayers = 0;
      for (var group in groups) {
      ...
      print('Manual group payout validation failed: $e');
      return null;
    }
  }
}