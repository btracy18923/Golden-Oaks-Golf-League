...
class _WednesdayEnterScoresScreenState extends State<WednesdayEnterScoresScreen> {
  ...
  Widget _buildTitleContainer() {
    ...
    final spacing = DeviceDetectionService.isPhone(context) ? 10.0 : 30.0;
  ...
  Widget _buildColumnHeaders() {
    final headerFontSize = ResponsiveTypography.getTableHeader(context);
    final isPhone = DeviceDetectionService.isPhone(context);
    final headerStyle = TextStyle(fontSize: headerFontSize, fontWeight: FontWeight.bold);

    // Responsive column widths
    final nameWidth = isPhone ? 80.0 : 120.0;
    final hcWidth = isPhone ? 35.0 : 50.0;
    final grossWidth = isPhone ? 45.0 : 60.0;
    final groupWidth = isPhone ? 45.0 : 60.0;
    final netWidth = isPhone ? 30.0 : 40.0;
    final avgWidth = isPhone ? 35.0 : 50.0;
    final posWidth = isPhone ? 35.0 : 50.0;
    final moneyWidth = isPhone ? 55.0 : 80.0;
    ...
    headers.add(Container(
      width: nameWidth,
      child: Text('Name', style: headerStyle, textAlign: TextAlign.center),
    ));
    ...
    if (selectedLeague == 'wednesday' && !groupsProcessed) {
      headers.add(Container(
        width: hcWidth,
        child: Text('HC', style: headerStyle, textAlign: TextAlign.center),
      ));
    }
    ...
    if (!groupsProcessed && selectedLeague != 'monday') {
      headers.add(Container(
        width: grossWidth,
        child: Text('Gross', style: headerStyle, textAlign: TextAlign.center),
      ));
    }
    ...
    if (selectedLeague == 'wednesday' && groupsProcessed) {
      headers.add(Container(
        width: groupWidth,
        child: Text('Group#', style: headerStyle, textAlign: TextAlign.center),
      ));
    }
    ...
    if (selectedLeague == 'wednesday') {
      headers.add(Container(
        width: netWidth,
        child: Text('Net', style: headerStyle, textAlign: TextAlign.center),
      ));
      ...
      if (groupsProcessed) {
        headers.add(Container(
          width: avgWidth,
          child: Text('AVG', style: headerStyle, textAlign: TextAlign.center),
        ));
      }

      headers.add(Container(
        width: posWidth,
        child: Text('Pos', style: headerStyle, textAlign: TextAlign.center),
      ));
      headers.add(Container(
        width: moneyWidth,
    ...
    if (selectedLeague == 'monday' && !groupsProcessed) {
      headers.add(Container(
        width: 70,
        child: Text('SKAT#', style: headerStyle, textAlign: TextAlign.center),
      ));
      headers.add(Container(
        width: 70,
        child: Text('SKATS', style: headerStyle, textAlign: TextAlign.center),
      ));
      headers.add(Container(
        width: 70,
        child: Text('DIFF', style: headerStyle, textAlign: TextAlign.center),
      ));
      headers.add(Container(
        width: 70,
        child: Text('\$\$\$', style: headerStyle, textAlign: TextAlign.center),
  ...
  Widget _buildPlayerRow(Map<String, dynamic>? player, int groupIndex, int rowIndex) {
    final isPhone = DeviceDetectionService.isPhone(context);
    final cellFontSize = ResponsiveTypography.getSmall(context);
    final rowHeight = isPhone ? 32.0 : 40.0;

    // Responsive column widths (matching header widths)
    final nameWidth = isPhone ? 80.0 : 120.0;
    final hcWidth = isPhone ? 35.0 : 50.0;
    final grossWidth = isPhone ? 45.0 : 60.0;
    final groupWidth = isPhone ? 45.0 : 60.0;
    ...
    if (player != null) {
      ...
      if (selectedLeague == 'wednesday' && !groupsProcessed) {
        rowWidgets.add(Container(
          ...
          child: Center(child: Text(player['handicap'] != null ? player['handicap'].toStringAsFixed(1) : '', style: TextStyle(fontSize: cellFontSize))),
        ));
      }
      ...
      if (selectedLeague == 'monday' && !groupsProcessed) {
        rowWidgets.add(Container(
          ...
          height: rowHeight,
          decoration: BoxDecoration(border: Border.all()),
          child: Center(child: Text(player['skat_number']?.toString() ?? '', style: TextStyle(fontSize: cellFontSize))),
        ));
        ...
        rowWidgets.add(Container(
          ...
          child: Center(child: Text(
            ...
            style: TextStyle(fontSize: cellFontSize, fontWeight: FontWeight.bold),
          )),
        ));
      }
    } else {
      // Empty placeholders
      if (selectedLeague == 'wednesday' && !groupsProcessed) {
        rowWidgets.add(_buildEmptyPlaceholder(hcWidth)); // HC placeholder
      }
      if (!groupsProcessed && selectedLeague != 'monday') {
        rowWidgets.add(_buildEmptyPlaceholder(grossWidth)); // Gross placeholder
      }

      // Group# placeholder (Wednesday only, and only if groups are processed)
      if (selectedLeague == 'wednesday' && groupsProcessed) {
        rowWidgets.add(_buildEmptyPlaceholder(groupWidth)); // Group# placeholder
      }

      if (selectedLeague == 'wednesday') {
        rowWidgets.add(_buildEmptyPlaceholder(netWidth)); // Net placeholder

        // AVG placeholder (only when groups are processed)
        if (groupsProcessed) {
          rowWidgets.add(_buildEmptyPlaceholder(avgWidth)); // AVG placeholder
        }

        rowWidgets.add(_buildEmptyPlaceholder(posWidth)); // Pos placeholder
        rowWidgets.add(_buildEmptyPlaceholder(moneyWidth)); // $$$ placeholder
      }
  ...
  Widget _buildPlayerNameButton(Map<String, dynamic> player, int groupIndex) {
    final isPhone = DeviceDetectionService.isPhone(context);
    final nameFontSize = ResponsiveTypography.getSmall(context);
    final rowHeight = isPhone ? 32.0 : 40.0;
    final nameWidth = isPhone ? 80.0 : 120.0;
    final wcFontSize = isPhone ? 8.0 : 10.0;
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Padding(
          padding: EdgeInsets.symmetric(horizontal: isPhone ? 2 : 4, vertical: isPhone ? 4 : 8),
          child: Row(
            children: [
              if (isWildCard) ...[
                Container(
  ...
  Widget _buildScoreInput(Map<String, dynamic> player) {
    final isPhone = DeviceDetectionService.isPhone(context);
    final inputFontSize = ResponsiveTypography.getSmall(context);
    final rowHeight = isPhone ? 32.0 : 40.0;
    final grossWidth = isPhone ? 45.0 : 60.0;
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          border: OutlineInputBorder(
            borderRadius: BorderRadius.circular(isPhone ? 4 : 8),
            borderSide: BorderSide(color: Colors.black, width: isPhone ? 1 : 2),
          ),
          contentPadding: EdgeInsets.symmetric(horizontal: isPhone ? 4 : 8, vertical: isPhone ? 2 : 4),
          counterText: '',
        ),
      ),
    );
  ...
  Widget _buildNetScoreLabel(Map<String, dynamic> player) {
    ...
    return Container(
      ...
      height: rowHeight,
      decoration: BoxDecoration(border: Border.all()),
      child: Center(child: Text(netText, style: TextStyle(fontSize: cellFontSize))),
    );
  }
  ...
  Widget _buildAvgLabel(int groupIndex) {
    ...
    return Container(
      ...
      height: rowHeight,
      decoration: BoxDecoration(border: Border.all()),
      child: Center(child: Text(avgText, style: TextStyle(fontSize: cellFontSize))),
    );
  }
  ...
  Widget _buildPlaceLabel(Map<String, dynamic> player, int groupIndex) {
    ...
    return Container(
      ...
      height: rowHeight,
      decoration: BoxDecoration(border: Border.all()),
      child: Center(child: Text(placeText, style: TextStyle(fontSize: cellFontSize))),
    );
  }
  ...
  Widget _buildIndWinLabel(Map<String, dynamic> player) {
    ...
    return Container(
      ...
      height: rowHeight,
      decoration: BoxDecoration(border: Border.all()),
      child: Center(child: Text(indText, style: TextStyle(fontSize: cellFontSize))),
    );
  }
  ...
  Widget _buildFooter() {
    final isPhone = DeviceDetectionService.isPhone(context);
    final footerHeight = isPhone ? 55.0 : 70.0;
    final horizontalPadding = isPhone ? 2.0 : 5.0;
  ...
  Widget _buildConditionalProcessGroupsButton() {
    ...
    final horizontalPadding = isPhone ? 3.0 : 10.0;
    final verticalPadding = isPhone ? 4.0 : 8.0;
    ...
    return Expanded(
      child: Padding(
        ...
        child: ElevatedButton(
          ...
          child: FittedBox(
            fit: BoxFit.scaleDown,
            child: Text(text, style: TextStyle(fontSize: buttonFontSize, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
          ),
        ),
      ),
    );
  ...
  Widget _buildSwapButton() {
    final isPhone = DeviceDetectionService.isPhone(context);
    final swapFontSize = isPhone ? 8.0 : 12.0;
    final horizontalPadding = isPhone ? 3.0 : 10.0;
    final verticalPadding = isPhone ? 4.0 : 8.0;
    ...
    return Expanded(
      child: Padding(
        ...
        child: ElevatedButton(
          ...
          child: FittedBox(
            fit: BoxFit.scaleDown,
            child: Text(buttonText, style: TextStyle(fontSize: swapFontSize, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
          ),
        ),
      ),
    );
  ...
  Widget _buildPlayerClickColumn(
    ...
    final isPhone = DeviceDetectionService.isPhone(context);
    final dialogFontSize = isPhone ? 12.0 : 16.0;
    final nameWidth = isPhone ? 80.0 : 120.0;
    final valueBoxSize = isPhone ? 24.0 : 30.0;
    ...
    return Column(
      ...
      children: [
        ...
        ...columnPlayers.map((player) {
          ...
          return Padding(
            padding: EdgeInsets.symmetric(vertical: isPhone ? 1 : 2),
            child: Row(
              children: [
                ...
                SizedBox(
                  ...
                  child: GestureDetector(
                    ...
                    child: Container(
                      padding: EdgeInsets.symmetric(vertical: isPhone ? 2 : 4, horizontal: 2),
                      decoration: BoxDecoration(
                ...
                SizedBox(width: isPhone ? 4 : 8), // Padding between name and value display
  ...
  Future<bool?> _showWinningsPopup(List<Map<String, dynamic>> playerScores, Map<String, int> playerValues) async {
    final isPhone = DeviceDetectionService.isPhone(context);
    final titleFontSize = isPhone ? 14.0 : 18.0;
    final contentFontSize = isPhone ? 12.0 : 14.0;
    final smallFontSize = isPhone ? 10.0 : 12.0;
    final totalFontSize = isPhone ? 14.0 : 16.0;
              ...
              children: [
                if (winnersData.isEmpty)
                  Padding(
                    ...
                    child: Text(
                      'No winners selected',
                      style: TextStyle(fontSize: contentFontSize, fontStyle: FontStyle.italic),
                    ),
                  )
                else ...[
                  Text(
                  ...
                  Text(
                    ...
                    style: TextStyle(fontSize: totalFontSize, fontWeight: FontWeight.bold, color: Colors.green.shade600),
                    textAlign: TextAlign.center,
                  ),
                ],
              ],
  ...
  Future<bool?> _showPlayersCheckboxDialog(List<Map<String, dynamic>> playerScores) async {
    final isPhone = DeviceDetectionService.isPhone(context);
    final titleFontSize = isPhone ? 14.0 : 18.0;
    final subtitleFontSize = isPhone ? 11.0 : 14.0;
    final columnSpacing = isPhone ? 4.0 : 8.0;
              ...
              title: Column(
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(fontSize: subtitleFontSize, fontWeight: FontWeight.w500, color: runningTotal == targetTotal ? Colors.green : Colors.blue),
                  ),
                ],
              ),
              content: Container(
